...
async function verifyTwilioSignature(req, log) {
...
export async function twilioWebhookHandler(fastify) {
    fastify.post("/webhooks/twilio", async (request, reply) => {
        ...
        try {
            ...
            const finalState = await processIncomingMessage(conversation.id, providerMessageId, requestId, {
                content,
                payload: request.body,
            });
