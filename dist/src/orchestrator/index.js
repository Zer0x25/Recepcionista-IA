...
export async function processIncomingMessage(conversationId, providerMessageId, requestId, inboundMessageData) {
    ...
    try {
        ...
        if (inboundMessageData) {
            ...
            currentState = await transitionState(conversationId, currentState, State.CLASSIFYING, providerMessageId, requestId);
        }
        ...
        if (isValidTransition(currentState, nextState)) {
            currentState = await transitionState(conversationId, currentState, nextState, providerMessageId, requestId, nextState === State.HANDOFF
        ...
        if (currentState === State.ANSWERING &&
            ...
            currentState = await transitionState(conversationId, currentState, State.WAITING_USER, providerMessageId, requestId);
        }
        orchestratorLogger.info({
...
async function transitionState(conversationId, from, to, providerMessageId, requestId, reason) {
