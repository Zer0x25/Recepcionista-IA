...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/admin/metrics/summary", async (request, _reply) => {
    const start = Date.now();
    const log = (request as any).adminLog;

    const [backlogSize, collisionRate, ttlExpiryRate, sendSuccessRate] = await Promise.all([
  ...
  fastify.get("/admin/jobs/backlog", async (request, _reply) => {
    const start = Date.now();
    const log = (request as any).adminLog;
  ...
  fastify.get("/admin/worker/heartbeat", async (request, _reply) => {
    const start = Date.now();
    const log = (request as any).adminLog;
